...
var platform_underneart_the_player = null
...
func handle_ladder_climbing(collision, delta):
	if !can_climb:
		return
	var climb_direction = Input.get_axis("mover_abajo", "mover_arriba")
	...
	if climb_direction == -1 && collision && collision.get_collider() is Platform:
		...
		if platform.can_be_disabled:
			platform_underneart_the_player = platform
			platform_underneart_the_player.disable_collision()
	
	if climb_direction == 1 && platform_underneart_the_player:
		platform_underneart_the_player.enable_collision()
		platform_underneart_the_player = null
